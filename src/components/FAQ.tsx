...
const faqs = [
  ...
  {
    question: "Is Raahi available in my city?",
    answer:
      "Raahi is currently available in 50+ cities across India and expanding rapidly. Check the app to see if your city is covered. If not, you can join our waitlist and we'll notify you when we launch in your area!",
  },
  {
          ...
          <Accordion type="single" collapsible className="space-y-4">
            {faqs.map((faq, index) => (
              <AccordionItem
                key={index}
                value={`item-${index}`}
                className="border border-border rounded-lg px-4 md:px-6 py-2 bg-card shadow-sm hover:shadow-md transition-shadow"
              >
                <AccordionTrigger className="text-left font-semibold text-base md:text-lg hover:no-underline py-4 md:py-6">
                  {faq.question}
                </AccordionTrigger>
        ...
        <div className="mt-12 md:mt-16 text-center">
          <div className="bg-card border border-border rounded-xl p-6 md:p-8 max-w-2xl mx-auto">
            ...
            <div className="flex flex-col sm:flex-row gap-3 md:gap-4 justify-center">
              ...
              <button className="px-6 py-3 border border-border rounded-lg font-medium hover:bg-muted transition-colors text-sm md:text-base">
                Join Community
              </button>
            </div>
          </div>
